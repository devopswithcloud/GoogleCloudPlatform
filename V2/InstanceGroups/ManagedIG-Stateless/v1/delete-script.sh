...
if gcloud compute forwarding-rules describe $FORWARDING_RULE_NAME --region=$REGION --quiet > /dev/null 2>&1; then
    ...
    gcloud compute forwarding-rules delete $FORWARDING_RULE_NAME --region=$REGION --quiet
fi
...
if gcloud compute target-http-proxies describe $TARGET_HTTP_PROXY_NAME --region=$REGION --quiet > /dev/null 2>&1; then
    ...
    gcloud compute target-http-proxies delete $TARGET_HTTP_PROXY_NAME --region=$REGION --quiet
fi

# Delete URL map
if gcloud compute url-maps describe $URL_MAP_NAME --region=$REGION --quiet > /dev/null 2>&1; then
    echo "Deleting URL map $URL_MAP_NAME..."
    gcloud compute url-maps delete $URL_MAP_NAME --region=$REGION --quiet
fi
...
if gcloud compute instance-groups managed describe $MANAGED_INSTANCE_GROUP_NAME --zone=$ZONE --quiet > /dev/null 2>&1; then
    echo "Deleting managed instance group $MANAGED_INSTANCE_GROUP_NAME..."
    gcloud compute instance-groups managed delete $MANAGED_INSTANCE_GROUP_NAME --zone=$ZONE --quiet
fi
